...
export async function saveProblem(dir, { topic, materials, templateName, roles, mode, maxRounds, lang = 'zh' }) {
  // problem.md 归档后 resume 时会整体作为 materials 读回进入提示词，所以头部要跟会话语言
  await w(path.join(dir, 'problem.md'), lang === 'en'
    ? `# Topic\n${topic}\n\n# Background\n${materials || '(none)'}\n\n# Config\n- Template: ${templateName}\n- Debaters: ${roles.debaters.join(', ')}\n- Judge: ${roles.judge}\n- Scribe: ${roles.summarizer}\n- Mode: ${mode}\n- Max rounds: ${maxRounds}\n`
...
export async function assembleSessionMd(dir) {
  ...
  // session.md 是给用户看的导出——标题跟会话语言（metadata.lang）
  let en = false;
  try { en = JSON.parse(await readFile(path.join(dir, 'metadata.json'), 'utf8')).lang === 'en'; } catch { /* 无 metadata 按中文 */ }
  ...
  for (const r of rounds) {
    ...
    for (const f of raws.filter(x => x.startsWith(r + '-') && !x.includes('summary')))
      parts.push(`## ${f.replace('.md', '').replace(r + '-', '')}${en ? "'s statement" : ' 发言'}\n\n${await tryRead(path.join(dir, 'raw', f))}`);
  ...
  if (dis) parts.push(en ? `\n---\n\n# Disagreement table (cumulative)\n\n${dis}` : `\n---\n\n# 分歧分类表（全场累计）\n\n${dis}`);
  const judgeRaws = raws.filter(f => f.startsWith('judge-'));
  if (judgeRaws.length > 0) {
    parts.push(en ? `\n---\n\n# Judge's statement (raw)` : `\n---\n\n# 仲裁发言原文`);
