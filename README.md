...
- **Workbench** (everyday): pick models and just chat. Unaddressed messages route to whoever spoke last; address one model or broadcast to all. Hit "let them talk" and the models take turns responding to each other for N rounds — encouraged to push back and question each other by name, auto-stopping when the discussion converges. Long histories are trimmed whole-message with a visible "this model only sees the last N messages" chip — never silently. One click promotes the chat into a formal committee meeting.
  - Each participant enters with a **capability** — **Discussant** (talks, reviews, never touches files) or **Proposer** (can be assigned changes — edits real files in an isolated copy, producing a diff you approve per file) — plus an optional stackable **Arbiter** duty on either: on conflicts it digests pro/con for your call and executes the merge (Discussant+Arbiter = a pure judge with no proposals of its own; an explicit "decide for me" tier can delegate the pick — applying to your workspace always stays yours). Engines without a vetted safe-write mode (no `writeArgs` in their adapter) are discussant-only — the isolated copy contains the blast radius, but an unfenced CLI could still run arbitrary commands inside it, so no fence, no write.
...
Stop the running Roundtable service, then update to the latest release tag and restart it:
...
`adapters/agents.json` is your own configuration file and is not overwritten by updates. A fresh checkout creates it from `adapters/agents.example.json` on first start.
...
- **工作台**（高频日常）：勾选模型直接群聊，随时追问、点名任意模型接力，还能让模型之间就你的话题**互聊 N 轮**（互相反驳、追问，聊无可聊自动收敛）
  - 每位参与者入场即定**能力**——**讨论者**（只动嘴，永不碰文件）或**提案者**（可被指派修改——在隔离副本真实改文件，产出 diff 由你逐文件批准）——并可叠加**仲裁**职责：冲突时整理 pro/con 供你拍板并执行融合（「讨论者+仲裁」= 不提自己方案的纯裁判；可显式授「替我决断」档——但应用进工作区的批准权永远在你）。未声明安全写模式（adapter 无 `writeArgs`）的引擎只能当纯讨论者：隔离副本管住改动外溢，安全写模式管住过程越权，缺一层就不给写权。
- **会议**（低频重决策）：完整委员会流程，见下
...
**参会的不是裸模型，是你调教过的完整 agent。** Roundtable 骑的是 CLI 而非 API——每位参会者自带你装好的技能（相关时自动触发）、你的记忆文件与全局指令，挂上项目目录还能翻真实代码而不是凭想象辩论。API 型「模型委员会」请到的是出厂模型，Roundtable 请到的是你已经养成的那几位。
...
git clone https://github.com/ai-us-stock-lab/roundtable
cd roundtable
git checkout "$(git describe --tags --abbrev=0)" # 切到最新 release tag
...
先停止正在运行的 Roundtable 服务，再切到最新 release tag 并重启：
...
`adapters/agents.json` 是用户自有配置，更新不会覆盖。全新检出时，首次启动会从 `adapters/agents.example.json` 自动生成。
