...
process.stdin.on('end', () => {
  const isBuild = /隔离副本|在 README 末尾新增一行/.test(input);
  if (isBuild) {
    const readme = path.join(process.cwd(), 'README.md');
    const line = 'Roundtable 让多个 AI CLI 在本机协作，并由用户审批每一次文件改动。';
    const existing = fs.readFileSync(readme, 'utf8');
    if (!existing.includes(line)) fs.appendFileSync(readme, `\n${line}\n`, 'utf8');
    process.stdout.write('已在 README.md 末尾补充项目一句话简介，等待你审批 diff。');
    return;
  }
