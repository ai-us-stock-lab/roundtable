...
**目标**：一个独立浏览器窗口，让两个（或更多）AI CLI 智能体就同一议题进行结构化辩论与协作，用户作为主持人全程可控，最终产出可留痕的裁决卡。首个用例是"南添决策双引擎裁决"，但架构是通用的。
...
## 6. Rolling Summary（用户要求 #4）

每轮结束由 summarizer 生成结构化摘要，**下一轮只传 rolling summary + 最新一轮原文**，完整历史绝不整体进上下文。摘要固定六栏：
...
所有轮次的 summary 与原文都持久化在会话文件中（供人查阅），只是不进上下文。
...
- `sessions/<日期>-<slug>.json`：结构化全量（轮次、原文、summary、裁决卡、错误记录）
- `sessions/<日期>-<slug>.md`：人读版（辩论全文 + 裁决卡）
