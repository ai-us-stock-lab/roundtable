...
### Task 11: 真实 CLI 接入（claude + codex，含 CLI 路径解析器）

**Files:**
- Create: `src/resolve.js`（CLI 路径解析器）
- Modify: `adapters/agents.json`、`src/server.js`（启动时解析 command[0]）
...
- Consumes: `runAgent`
- Produces:
  - `resolveCliPath(cfg) => string`——按优先级解析 command[0]：①`cfg.commandEnvVar` 指定的环境变量（如 `CODEX_CLI_PATH`）存在且文件存在→用它；②command[0] 本身是存在的绝对路径→直接用；③PATH 可找到（Windows 遍历 PATHEXT）→用之；④`cfg.commandFallbackGlob`（如 `~/AppData/Local/OpenAI/Codex/bin/*/codex.exe`）命中→取 **mtime 最新**（版本哈希目录随升级变化）；⑤都没有→抛带安装指引的错误
  - server 启动时对每个 agent 解析并替换 command[0]，失败的 agent 标记 `unavailable` 并在 `/api/config` 中暴露（前端下拉禁用该项），服务照常启动
  - agents.json 增加 `claude` 与 `codex` 条目；`node scripts/smoke.js <agentId>` 冒烟

**已实测事实（2026-07-14 本机，实现时直接采信）**：
- `CODEX_CLI_PATH` = `C:\Users\xiaoJ\AppData\Local\OpenAI\Codex\bin\<hash>\codex.exe`（Codex Desktop 设置的用户级环境变量，Node 可读）；当前版本 `codex-cli 0.142.0-alpha.1`
- `codex exec`：无参数时 prompt 从 stdin 读 ✅；`--sandbox read-only` ✅；`--ephemeral`（不持久化会话）✅；`--json`（JSONL 事件）与 `-o <file>` 可选
- codex.exe 是原生 exe，`spawn(绝对路径, args, {shell:false})` 直接可用，无 .cmd 包装问题
- bin 下存在多个版本哈希目录 → 必须走解析器，禁止把哈希路径写死进 agents.json

- [ ] **Step 1: 写 resolve.js 失败测试**
...
- [ ] **Step 3: 实现 resolve.js 并接入 server**
...
```
`/api/config` 的 agents 输出加 `unavailable` 字段；前端 `boot()` 对 unavailable 的 agent 在 option 上加 `disabled` 并在名字后缀「(不可用)」。

- [ ] **Step 4: 实测验证 claude 旗标 + codex 输出形态**

```bash
claude --help
```
逐项确认（以实际输出为准，不符则修正 agents.json 并在 README 记录）：`-p/--print`；`--output-format stream-json` 是否需搭配 `--verbose`；禁用工具的准确旗标（候选 `--disallowedTools`，不支持通配符就逐个列 `Bash,Edit,Write,NotebookEdit,WebFetch,WebSearch,Glob,Grep,Read,Task`）。claude 在 Windows 上可能是 .cmd 包装，resolve.js 的 PATH 查找已覆盖 PATHEXT；若 spawn 仍 ENOENT，改用 `['cmd','/c','claude',...]` 形态并记录进 README。

codex 输出形态确认：跑 `echo 1+1=? | codex exec --sandbox read-only --ephemeral` 看裸 stdout 是否干净；若混有进度噪音，改用 `--json` 并抓一份真实 JSONL 样本，在 `extractStreamText` 中加 codex 最终消息事件的识别分支（以样本为准写断言，加进 runner.test.js）。

- [ ] **Step 5: 写冒烟脚本（经 resolveCliPath）**
...
- [ ] **Step 6: 更新 agents.json（以 Step 4 实测修正）**
...
```json
{
  ...
  "codex": {
    ...
    "commandEnvVar": "CODEX_CLI_PATH",
    "commandFallbackGlob": "~/AppData/Local/OpenAI/Codex/bin/*/codex.exe",
    "input": "stdin",
    "output": "text",
    "timeoutMs": 300000,
    "envWhitelist": ["PATH", "USERPROFILE", "HOME", "APPDATA", "LOCALAPPDATA", "SYSTEMROOT", "COMSPEC", "TEMP", "TMP"],
...
```
注意：codex 是原生 exe，spawn 无障碍；claude 若是 npm 全局 .cmd 包装，`spawn('...cmd', args, {shell:false})` 可能报 EINVAL/ENOENT——实测若失败，改 `['cmd','/c','claude',...]` 形态；**以实测通过为准，把最终可用形态写进 README 与 agents.json**。
...
```
Expected: 两个都 `ok: true`，text 含"2"。codex 首次运行若报未登录，请用户在终端跑一次 codex 登录后重试。

- [ ] **Step 8: Commit**

```bash
git add adapters/agents.json scripts/smoke.js src/resolve.js test/resolve.test.js src/server.js
git commit -m "feat: CLI 路径解析器 + claude/codex 真实接入与冒烟"
```
