...
test('Workbench.note: meeting 元数据随消息落盘并随 chat-message emit', async () => {
  const baseDir = mkdtempSync(path.join(tmpdir(), 'wb-note-'));
  const events = [];
  const w = new Workbench({ name: '会议时间线', agents: MOCK_AGENTS(), participants: ['m1'], baseDir, emit: e => events.push(e) });
  await w.init();
  const meeting = { dir: '2026-07-21-meeting', topic: '结构化会议卡', kind: 'started' };

  await w.note('会议已开始', { meeting });
  await w.note('普通时间线消息');

  const { messages } = await loadWorkbenchFromDisk(w.dir);
  assert.deepEqual(messages[0].meeting, meeting);
  assert.equal(messages[1].meeting, undefined);
  assert.deepEqual(events.find(e => e.type === 'chat-message' && e.data === '会议已开始')?.meeting, meeting);
  assert.equal(events.find(e => e.type === 'chat-message' && e.data === '普通时间线消息')?.meeting, undefined);
});
...
test('server: 工作区动作校验路径、重派生能力并持久化挂载/卸载', async () => {
  ...
  try {
    ...
    assert.match((await missingResponse.json()).error, /找不到这个路径/);
...
// ---- 统一容器：孤儿会议自动包台 + 裁决卡回流 ----
test('server: 无 origin 建会自动包台，并在时间线写入结构化 started 会议卡', async () => {
  const { startServer } = await import('../src/server.js');
  const sessionsDir = mkdtempSync(path.join(tmpdir(), 'wb-wrap-'));
  const srv = await startServer({ port: 0, agentsFile: 'test/agents.fixture.json', sessionsDir });
  const base = `http://127.0.0.1:${srv.port}`;
  const post = (p, body) => fetch(base + p, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(body ?? {}) }).then(r => r.json());
  try {
    const topic = '孤儿会议自动包台-' + '长'.repeat(40);
    const sess = await post('/api/sessions', {
      topic, materials: '', template: 'general',
      roles: { debaters: ['mockA', 'mockB'], judge: 'mockA', summarizer: 'mockB' }, mode: 'manual',
    });
    assert.ok(sess.id);
    assert.equal(srv.benches.size, 1);

    const committee = srv.sessions.get(sess.id).committee;
    const meetingDirname = path.basename(committee.dir);
    assert.ok(committee.origin);
    const wrapEntry = [...srv.benches.values()].find(e => path.basename(e.bench.dir) === committee.origin);
    assert.ok(wrapEntry, '自动包裹工作台应处于活动列表');
    assert.equal(wrapEntry.bench.name, topic.slice(0, 40));
    assert.deepEqual(wrapEntry.bench.participants, ['mockA', 'mockB']);

    const list = await fetch(base + '/api/sessions').then(r => r.json());
    const meetingItem = list.find(s => s.id === sess.id);
    const benchItem = list.find(s => s.type === 'workbench' && s.dirname === committee.origin);
    assert.equal(meetingItem.origin, committee.origin);
    assert.equal(meetingItem.dirname, meetingDirname);
    assert.equal(benchItem.topic, '[工作台] ' + topic.slice(0, 40));

    const started = wrapEntry.bench.messages.find(m => m.meeting?.kind === 'started');
    assert.ok(started);
    assert.match(started.text, /会议已开始/);
    assert.deepEqual(started.meeting, { dir: meetingDirname, topic, kind: 'started' });
  } finally { srv.close(); }
});
...
test('flowback: 升格携带来源，裁决卡回流贴回原工作台（含从磁盘恢复路径）', async () => {
  ...
  try {
    ...
    const benchCountBeforeMeeting = srv.benches.size;
    // 升格草稿带来源
    ...
    assert.ok(sess.id);
    assert.equal(srv.benches.size, benchCountBeforeMeeting, '已有 origin 时不应重复包台');
    const detail = await fetch(`${base}/api/sessions/${sess.id}`).then(r => r.json());
    assert.equal(detail.origin, benchDirname);
    // 统一容器：开会即在来源工作台时间线留痕
    const started = srv.benches.get(wbid).bench.messages.at(-1);
    assert.match(started.text, /会议已开始/);
    assert.deepEqual(started.meeting, { dir: path.basename(detail.dir), topic: detail.topic, kind: 'started' });
    // 无裁决卡时手动投放被拒
    ...
    assert.deepEqual(auto.meeting, { dir: path.basename(detail.dir), topic: detail.topic, kind: 'verdict' });
    // 手动投放路径仍可用（显式再分发）
    ...
    assert.ok(srv.benches.get(fb2.benchId).events.some(e => e.type === 'chat-message' && e.meeting?.kind === 'verdict'), '恢复回放应保留 meeting 元数据');
  } finally { srv.close(); }
});
