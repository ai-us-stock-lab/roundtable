...
const I18N = {
  ...
  // ---- 引擎状态灯 ----
  'status.engines': { zh: 'Agent 状态', en: 'Agents' },
  ...
  'wb.roleNoWrite': { zh: '该 agent 无安全写模式，只能作为纯讨论者（提案与执行融合都需写文件）', en: 'This engine has no safe write mode — pure discussant only (both proposing and merge execution write files)' },
  ...
  'wb.helpTalk': { zh: '讨论者：不自主产出变更——只讨论与评审。角色管的是 agent 的自主行为；你在下方显式指派任何有写能力的 agent（含讨论者）都可以。', en: 'Discussant: never produces changes on its own — discussion and review only. Roles govern autonomous behavior; you can still explicitly assign any write-capable engine (discussants included) below.' },
  ...
  'diag.toggleTip': { zh: '展开或收起 Agent 诊断与修复指引', en: 'Show or hide engine diagnostics and recovery guidance' },
  ...
  'diag.timeout': { zh: '响应超时——Agent 可能较慢，可点灯重试；持续超时可在 adapters/agents.json 调大 timeoutMs（重启生效）', en: 'Response timed out. The engine may be slow; click its light to retry. If timeouts persist, increase timeoutMs in adapters/agents.json and restart the service.' },
  ...
  'setup.browse': { zh: '浏览…', en: 'Browse…' },
  'folder.pick': { zh: '选择此文件夹', en: 'Use this folder' },
  'folder.up': { zh: '↑ 上级', en: '↑ Parent' },
  'folder.cancel': { zh: '取消', en: 'Cancel' },
  'folder.title': { zh: '选择项目目录', en: 'Choose project directory' },
  'folder.loading': { zh: '正在读取文件夹…', en: 'Loading folders…' },
  'folder.empty': { zh: '没有可进入的子文件夹，可直接选择当前文件夹。', en: 'No subfolders here. You can use the current folder.' },
  'browse.notDir': { zh: '不是有效的文件夹', en: 'Not a valid folder' },
  'agentcfg.open': { zh: '配置 Agent…', en: 'Configure agents…' },
  'agentcfg.title': { zh: '手工配置 Agent', en: 'Configure agents' },
  'agentcfg.hint': { zh: '保存后立即对新会话生效；进行中的会话不受影响。这是你本机的 adapters/agents.json。', en: 'Saving applies immediately to new sessions; sessions already in progress are not affected. This is your local adapters/agents.json.' },
  'agentcfg.save': { zh: '保存并生效', en: 'Save & apply' },
  'agentcfg.cancel': { zh: '取消', en: 'Cancel' },
  'agentcfg.saved': { zh: '已保存，新会话生效', en: 'Saved. Applies to new sessions.' },
