...
const I18N = {
  ...
  'wb.actionTitle': { zh: '🔧 动手（隔离副本 · diff 审批）', en: '🔧 Build (isolated copy · diff approval)' },
  'wb.actor': { zh: '执行者', en: 'Executor' },
  'wb.taskPh': { zh: '动手任务：要改什么、为什么、验收标准……（Ctrl+Enter 执行）', en: 'Task: what to change, why, acceptance criteria… (Ctrl+Enter to run)' },
  ...
  'wb.permPropose': { zh: '提案/动手', en: 'Propose/Build' },
  'wb.permProposeTip': { zh: '允许该模型动手产出 diff 提案。关闭后它从执行者列表消失，只能动嘴', en: 'Allow this model to build and produce diff proposals. Off = talk only, removed from the executor list' },
  'wb.permApply': { zh: '定稿', en: 'Finalize' },
  'wb.permApplyTip': { zh: '允许该模型执行定稿/融合（多 diff 合成最终方案）。定稿功能上线后生效；应用进工作区的批准权永远在你', en: 'Allow this model to run finalize/merge (compose multiple diffs into the final change). Takes effect when finalize ships; applying to your workspace always needs your approval' },
  'wb.permNoCapability': { zh: '该引擎无安全写模式，权限不可开启', en: 'This engine has no safe write mode — permissions cannot be enabled' },
  'wb.noEligibleActor': { zh: '（无已授权执行者——先在上方开启某位的「提案/动手」）', en: '(no authorized executor — enable someone\'s Propose/Build above)' },
